...
class TestCredoHolderStartup:
    ...
    def test_start_allocates_port_and_starts_subprocess(self):
        with patch("agents.holder.credo.subprocess.Popen") as mock_popen, \
                ...
                patch("agents.holder.credo.CredoHolder.read_json_line",
    ...
    def test_start_returns_port_on_failure(self):
        with patch("agents.holder.credo.subprocess.Popen", side_effect=OSError("exec error")), \
...
class TestCredoHolderShutdown:
    ...
    def test_shutdown_safe_on_communicate_timeout(self):
        with patch("agents.holder.credo.portmanager.return_port"), \
                patch("agents.holder.credo.os.kill") as mock_kill:
            from agents.holder.credo import CredoHolder
            holder = CredoHolder()
            holder.port = 10008
...
class TestCredoHolderCommands:
    ...
    def test_accept_invite_sends_receive_invitation(self):
        with patch("agents.holder.credo.CredoHolder.run_command") as mock_run, \
                patch("agents.holder.credo.CredoHolder.read_json_line",
                      return_value={"connection": "conn-1"}):
            from agents.holder.credo import CredoHolder
            holder = CredoHolder()
    ...
    def test_accept_invite_fallback_on_connection_did(self):
        with patch("agents.holder.credo.CredoHolder.run_command") as mock_run, \
                patch("agents.holder.credo.CredoHolder.read_json_line",
                      return_value={"connection": "conn-2"}):
            from agents.holder.credo import CredoHolder
            holder = CredoHolder()
    ...
    def test_accept_invite_returns_none_when_no_connection(self):
        with patch("agents.holder.credo.CredoHolder.run_command"), \
                patch("agents.holder.credo.CredoHolder.read_json_line",
                      return_value={"no_connection": True}):
            from agents.holder.credo import CredoHolder
            holder = CredoHolder()
